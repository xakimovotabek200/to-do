...
const EditModal = () => {
    ...
    const handleUpdateTodo = async (todoId) => {
        try {
            const response = await axios.get(
                `http://192.168.1.195:4000/todo/update/${todoId}`,
                {
                    headers: {
                        Authorization: token,
            ...
            setUpdateTodoData(response.data);
            showModal(todoId);
        } catch (error) {
            console.error("Error updating todo:", error);
            message.error("Error updating todo. Please try again.");
        }
    };
    ...
    return (
        <div>
            ...
            <Form name="form" onFinish={handleUpdateTodo} className="grid grid-cols-2 gap-10">
